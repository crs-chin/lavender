...
typedef struct _rpclite_svc rpclite_svc;
...
/**
 * NOTE: if @rsp is not called, the return value will be transacted to
 * the return value of client side @rpclite_transact, otherwise,
 * ignored.  the handler implementation should be able to distinguish
 * canonical and extended rpc, the canonical can't be responded
 * multiple times.
 */
typedef int (*rpclite_handler)(int req, const char *blob, size_t sz, rpclite_response rsp, void *rsp_ud, void *ud);

#define RPCLITE_RSP_F_END 1     /* last piece of response blob */
/* return zero to ignore subsequence blobs */
typedef int (*rpclite_cb)(void *rsp, size_t sz, int flags, void *ud);

struct _rpclite_svc{
    char *svc_name;
    rpclite_handler handler;
    void *ud;
    /* private */
    hlist node;
    int svc_id;
